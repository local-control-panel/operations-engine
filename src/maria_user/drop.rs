use crate::{
    ...
    maria_user::{DROP_OPERATION, DropRequest, DropResult},
    mutation::preflight,
    process::{
    ...
    pub fn protocol(&self) -> (ErrorCode, String) {
        match self {
            Self::Preflight(preflight::Error::Lock(_)) => (
                ErrorCode::Conflict,
                "another MariaDB user removal is in progress".into(),
            ),
            ...
            Self::Run(error) => (
                ...
                "could not run MariaDB user removal".into(),
            ),
            Self::Rejected(diagnostics) => (
                ...
                "MariaDB rejected the user removal".into(),
            ),
            Self::Cancelled => (
                ErrorCode::Cancelled,
                "cancelled before MariaDB user removal ran".into(),
            ),
            Self::Replayed { code, message } => (*code, message.clone()),
            _ => (
                ErrorCode::Internal,
                "internal MariaDB user removal error".into(),
            ),
        }
    }
}
...
pub fn execute(
    ...
    let scope = open(ctx.engine_state, &req.user, &req.host).map_err(Error::Io)?;
    ...
    let input = format!(
        "{}\nDROP USER '{}'@'{}';\n",
        req.root_password, req.user, req.host
    );
...
fn open(root: &ManagedRoot, user: &str, host: &str) -> std::io::Result<ManagedRoot> {
    let path = SiteRelativePath::parse(format!("maria-user-drop/{user}@{host}")).unwrap();
...
mod tests {
    ...
    #[test]
    fn credentials_stay_out_of_argv_and_drop_is_fixed() {
        ...
        fs::write(
            ...
            format!(
                ...
                argv_path.display(), stdin_path.display()
            ),
        ).unwrap();
        ...
        let request = DropRequest::parse(
            r#"{"container":"mariadb-11","rootPassword":"top-secret","user":"site_user","host":"%"}"#,
        ...
        assert!(input.contains("DROP USER 'site_user'@'%';"));
