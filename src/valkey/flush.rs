use crate::{
    ...
    valkey::{FLUSH_DB_OPERATION, FlushDbRequest, FlushResult},
};
    ...
    pub fn protocol(&self) -> (ErrorCode, String) {
        match self {
            Self::Preflight(preflight::Error::Lock(_)) => (
                ErrorCode::Conflict,
                "another Valkey database flush is in progress".into(),
            ),
            ...
            Self::Run(error) => (
                ...
                "could not run Valkey database flush".into(),
            ),
            Self::Rejected(diagnostics) => (
                ...
                "Valkey rejected the database flush".into(),
            ),
            Self::Cancelled => (
                ErrorCode::Cancelled,
                "cancelled before Valkey database flush ran".into(),
            ),
            Self::Replayed { code, message } => (*code, message.clone()),
            _ => (
                ErrorCode::Internal,
                "internal Valkey database flush error".into(),
            ),
        }
    }
}
...
fn open(root: &ManagedRoot, container: &str) -> std::io::Result<ManagedRoot> {
    let path = SiteRelativePath::parse(format!("valkey-flush-db/{container}")).unwrap();
...
fn replay(scope: &ManagedRoot, id: crate::transaction::RequestId) -> Result<FlushResult, Error> {
    ...
    if loaded.operation != FLUSH_DB_OPERATION {
...
mod tests {
    ...
    #[test]
    fn uses_only_the_fixed_flushdb_async_argv() {
        ...
        let request = FlushDbRequest::parse(
            r#"{"container":"valkey-8","confirmation":"FLUSHDB"}"#,
