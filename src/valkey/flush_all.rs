use crate::{
    ...
    valkey::{FLUSH_ALL_OPERATION, FlushAllRequest, FlushResult},
};
    ...
    pub fn protocol(&self) -> (ErrorCode, String) {
        match self {
            Self::Preflight(preflight::Error::Lock(_)) => (
                ErrorCode::Conflict,
                "another Valkey all-databases flush is in progress".into(),
            ),
            ...
            Self::Run(error) => (
                ...
                "could not run Valkey all-databases flush".into(),
            ),
            Self::Rejected(diagnostics) => (
                ...
                "Valkey rejected the all-databases flush".into(),
            ),
            Self::Cancelled => (
                ErrorCode::Cancelled,
                "cancelled before Valkey all-databases flush ran".into(),
            ),
            Self::Replayed { code, message } => (*code, message.clone()),
            _ => (
                ErrorCode::Internal,
                "internal Valkey all-databases flush error".into(),
            ),
        }
    }
}
...
pub fn execute(
    ...
    if !matches!(output.stdout.bytes.as_slice(), b"OK\n" | b"OK\r\n") {
        return Err(fail(
            &scope,
            &state_path,
            &audit_path,
...
fn open(root: &ManagedRoot, container: &str) -> std::io::Result<ManagedRoot> {
    let path = SiteRelativePath::parse(format!("valkey-flush-all/{container}")).unwrap();
...
fn replay(scope: &ManagedRoot, id: crate::transaction::RequestId) -> Result<FlushResult, Error> {
    let loaded = state::load(scope, &state_path(id))
        .map_err(|error| Error::Io(std::io::Error::other(format!("{error:?}"))))?;
    if loaded.operation != FLUSH_ALL_OPERATION {
...
mod tests {
    ...
    #[test]
    fn uses_only_the_fixed_flushall_async_argv() {
        ...
        let request = FlushAllRequest::parse(
            r#"{"container":"valkey-8","confirmation":"FLUSHALL"}"#,
