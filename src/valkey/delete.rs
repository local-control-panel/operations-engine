use crate::{
    ...
    valkey::{DELETE_OPERATION, DeleteRequest, DeleteResult},
};
    ...
    pub fn protocol(&self) -> (ErrorCode, String) {
        match self {
            Self::Preflight(preflight::Error::Lock(_)) => (
                ErrorCode::Conflict,
                "another deletion for this Valkey key is in progress".into(),
            ),
            ...
            Self::Run(error) => (
                ...
                "could not run Valkey key deletion".into(),
            ),
            Self::Rejected(diagnostics) => (
                ...
                "Valkey rejected the key deletion".into(),
            ),
            Self::Cancelled => (
                ErrorCode::Cancelled,
                "cancelled before Valkey key deletion ran".into(),
            ),
            Self::Replayed { code, message } => (*code, message.clone()),
            _ => (
                ErrorCode::Internal,
                "internal Valkey key deletion error".into(),
            ),
        }
    }
}
...
pub fn execute(
    ...
    let scope = open(ctx.engine_state, &req.key).map_err(Error::Io)?;
    ...
    let deleted = match output.stdout.bytes.as_slice() {
        b"0\n" | b"0\r\n" => false,
        b"1\n" | b"1\r\n" => true,
        _ => {
            return Err(fail(
...
fn open(root: &ManagedRoot, key: &str) -> std::io::Result<ManagedRoot> {
    let digest = Sha256::digest(key.as_bytes());
    let mut key_hash = String::with_capacity(digest.len() * 2);
    for byte in digest {
        write!(&mut key_hash, "{byte:02x}").expect("writing to a String cannot fail");
    }
    let path = SiteRelativePath::parse(format!("valkey-delete/{key_hash}")).unwrap();
...
fn replay(scope: &ManagedRoot, id: crate::transaction::RequestId) -> Result<DeleteResult, Error> {
    ...
    if loaded.operation != DELETE_OPERATION {
...
mod tests {
    ...
    #[test]
    fn key_stays_out_of_argv_and_is_sent_verbatim_over_stdin() {
        ...
        let key = "cache key\nключ*";
        let request = DeleteRequest::parse(
            &serde_json::json!({"container":"valkey-8","key":key}).to_string(),
        ...
        assert_eq!(fs::read(stdin_path).unwrap(), key.as_bytes());
        let argv = fs::read_to_string(argv_path).unwrap();
        assert!(argv.contains("valkey-cli\n-x\nDEL"));
        assert!(!argv.contains("cache key"));
    }
}
