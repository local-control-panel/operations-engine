...
- scheduled jobs — **target-aware `cron.installTab` shipped** (hash-guarded whole-crontab replacement via a staged file + argv-only `crontab [-u user] <path>`; target usernames are strictly validated and lock/transaction state is isolated per user). The control panel's general per-user cron writes plus backup/deploy/agent scheduling are engine-only; see `docs/milestones/007-target-aware-cron.md`;
