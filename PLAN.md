...
[`001-git-deploy-rollback.md`](https://github.com/local-control-panel/docs/blob/main/operations-engine/milestones/001-git-deploy-rollback.md).
...
audit pipeline. See the docs repo milestone `004-permissions-fix-ownership.md`.
...
the docs repo milestone `005-permissions-fix-world-writable.md`.
...
the docs repo milestone `006-mariadb-provisioning.md`.
...
when container removal fails. See the docs repo milestones
`008-db-tool-converge.md` and `009-db-tool-remove.md`.
...
- stack status and reconciliation — **`ingress.reconcile` and `runtime.reconcile` both shipped** (`docs repo milestone 002-ingress-reconcile.md`, `docs repo milestone 003-runtime-reconcile.md`; `src/ingress/reconcile.rs`, `src/runtime_config/reconcile.rs`, 300/300 passing, clippy and fmt clean). Together they cover two of `website-control-panel`'s `reconciliation.rs::sweep`'s seven orphan classes - `TempFile` and both `RedundantBackup`/`RecoverableBackup` variants - across both trusted roots. `ingress.reconcile` is a whole-`ingressRoot` sweep (no per-domain scoping needed - `ingressRoot` is flat); `runtime.reconcile` instead takes `--runtime-id` and sweeps only that pool's `<runtime_id>/` subdirectory of `runtimeRoot`, reusing `runtime_config::execute::open_runtime_config_state` verbatim so its lock/state scoping is identical to `runtime.activateConfig`'s per-`runtime_id` granularity rather than introducing a third, host-wide lock scope inconsistent with it - see the milestone doc's "Two differences" section for the full reasoning (a whole-`runtimeRoot` sweep in one call was considered and rejected for exactly that inconsistency). Deliberately narrowed, not everything at once: `OrphanedExecConfig`/`RouteWithoutExecConfig` need a two-root cross-reference neither operation's single-root scan does; `StaleLock` addresses `website-control-panel`'s own pre-engine `/etc/wcp/locks/` coordination directory, which this engine has no visibility into and shouldn't; `IdlePool` needs the site registry (which sites reference which runtime pool), which lives in that app's own SQLite. Wiring `website-control-panel`'s call sites for both operations is a separate, not yet started increment;
- backup and restore — **`db.restore` shipped, audit-trail scope only** (runs the existing `mariadb`/`psql` restore client against an already-on-disk dump via argv-only subprocess execution instead of a raw SSH shell string; the backup restore workflow plus direct `maria_import` and `pg_import` callers are now engine-only; no `HashGuard`, no pre-restore snapshot, no rollback - a destructive import has no "expected prior content" to guard and this was an explicit scope decision, not an oversight, deferring real transactional safety - snapshot + verify + auto-rollback-on-failure - as a materially larger follow-up on par with this engine's original deploy/rollback pipeline). Required two small additions to `process.rs` (`run_with_stdin_file`, `run_piped`) so a large dump file streams into the client via direct fd handoff/OS-level pipe rather than being buffered through this process - the first two callers here that need input larger than what `ProcessLimits`' bounded output capture was ever sized for;
- scheduled jobs — **target-aware `cron.installTab` shipped** (hash-guarded whole-crontab replacement via a staged file + argv-only `crontab [-u user] <path>`; target usernames are strictly validated and lock/transaction state is isolated per user). The control panel's general per-user cron writes plus backup/deploy/agent scheduling are engine-only; see the docs repo milestone `007-target-aware-cron.md`;
